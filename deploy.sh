...
# A misspelled LOG_STORE_ variable stops the new binary at boot. Ask before the
# old one is stopped: build the registry from the running service's own
# environment. The test prints a verdict, never a value.
